...
from jax_md.partition import NeighborList, neighbor_list_mask
...
d3_params_file = Path(__file__).with_name("param_dftd3.npz")
d3_params = jnp.load(d3_params_file)
...
def neighborlist_disp(displacement_fn: Callable):
    def distance_fn(positions: Array, neighbor: NeighborList, **kwargs):
        receivers, senders = neighbor.idx
        Rb = positions[senders]
        Ra = positions[receivers]
        mask = neighbor_list_mask(neighbor, True)
...
def _multi_iso_smoothing(cutoff: float, dr):
    ...
    return jnp.where(dr < r_onset, 1, inner)  # type: ignore
...
def _getc6(
    c6ab: Array, k3: float, Zi: Array, Zj: Array, nci: Array, ncj: Array
) -> Array:
    c6ab_ = c6ab[Zi, Zj]
    cn0 = c6ab_[:, :, :, 0]
    r = jnp.power((c6ab_[:, :, :, 1] - nci[:, None, None]), 2.0) + jnp.power(
        (c6ab_[:, :, :, 2] - ncj[:, None, None]), 2.0
    )
    n_edges = r.shape[0]
    ...
    if cn0.shape[0] == 0:
        ...
        k3_rnc = jnp.where(
            cn0 > 0.0, k3 * r, -1.0e20 * jnp.ones_like(r)
        ).reshape(n_edges, n_c6ab)
