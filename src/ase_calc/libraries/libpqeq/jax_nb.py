...
def taper(r: Array, swa: float, swb: float) -> Array:
    """Taper function for smooth cutoff. The taper smooth is close to Ewald summation."""  # noqa: E501
    r = r - swa
    rc = swb - swa
...
def dtaper(r: Array, swa: float, swb: float) -> Array:
    ...
    tap4 = -35.0 / jnp.power(rc, 4)
    return (
        7.0 * tap7 * jnp.power(r, 6)
        + 6.0 * tap6 * jnp.power(r, 5)
        + 5.0 * tap5 * jnp.power(r, 4)
        + 4.0 * tap4 * jnp.power(r, 3)
    )


def coulombev(r: Array, alpha: Array, cutoff: float) -> Array:
    return (
        jax.scipy.special.erf(alpha * r)
        * COULCONSTANTEV
        / r
        * taper(r, 0.0, cutoff)
    )
...
def coulombkcal(r: Array, alpha: Array, cutoff: float):
    return (
        jax.scipy.special.erf(alpha * r)
        * COULCONSTANTKCAL
        / r
...
def _dcoulombkcal(r: Array, alpha: Array, cutoff: float) -> Array:
...
def _sforce_dcoulombkcal(r: Array, alpha: Array, cutoff: float) -> Array:
...
def build_A_matrix(
    ...
    **kwargs,
    ...
    mat_A = mat_A.at[iidx, jidx].set(
        vmap_coulombkcal(scalar_dr, alpha[iidx, jidx], cutoff)
    )
    ...
    return mat_A, dR, scalar_dr  # type: ignore
...
def compute_shell(
    ...
    sforce = sforce.at[iidx].add(
        -vmap_sforce_dcoulobmkcal(dRss, alpha[iidx, jidx], cutoff)
        * qs[iidx][:, None]
        * qs[jidx][:, None]
    )
    sforce = sforce.at[iidx].add(
        -vmap_sforce_dcoulobmkcal(dRsc, alpha[iidx, jidx], cutoff)
        * qs[iidx][:, None]
        * qc[jidx][:, None]
    )
...
def build_b_vector(
    ...
    b = b.at[iidx].add(
        (
            -vmap_coulombkcal(scalar_dRcc, alpha[iidx, jidx], cutoff)
...
def pqeq_fori_loop(
    ...
    mat_A, dRcc, scalar_dRcc = build_A_matrix(
        displacement_fn, positions, neighbor, alpha, eta0, cutoff
    )
    r_shell = jnp.zeros_like(positions)
    if iterations == 1:
        vec_b = build_b_vector(
            dRcc,
            scalar_dRcc,
            positions,
            neighbor,
        ...
        r_shell = compute_shell(
            dRcc, positions, neighbor, r_shell, charges, alpha, z, Ks, cutoff
        )
        ...
        return charges, r_shell  # type: ignore


def nonbond_potential(
    displacement_fn: Callable,
    positions: Array,
    neighbor: partition.NeighborList,
    shell_positions: Array,
    ...
    **kwargs,
    ...
    pot = (
        jnp.dot(charges, chi0)
        + 0.5 * jnp.dot(charges**2, eta0)
        + 0.5
        * jnp.dot(jnp.sum(jnp.square(shell_positions), axis=1), Ks)
        / EV2KCALPMOL
    )
    ...
    pot += (
        jnp.sum(
            vmap_coulombev(dr, alpha[iidx, jidx], cutoff)
            * qc[iidx]
            * qc[jidx]
            * mask
        )
    ...
    pot += (
        jnp.sum(
            vmap_coulombev(space.distance(dRsc), alpha[iidx, jidx], cutoff)
            * qs[iidx]
            * qc[jidx]
            * mask
        )
    ...
    pot += (
        jnp.sum(
            vmap_coulombev(space.distance(dRss), alpha[iidx, jidx], cutoff)
            * qs[iidx]
            * qs[jidx]
            * mask
        )
    ...
    pot += (
        jnp.sum(
            vmap_coulombev(space.distance(dRcs), alpha[iidx, jidx], cutoff)
            * qc[iidx]
            * qs[jidx]
            * mask
        )
    ...
    if smooth_fn is not None:
        assert callable(smooth_fn), "smooth_fn must be callable."
